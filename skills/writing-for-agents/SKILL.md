...
description: >
  Writing documents agents consume: skills, AGENTS.md / CLAUDE.md, and docs
  reached by pointers. Use when creating or editing a skill, modifying
  AGENTS.md or CLAUDE.md, or restructuring agent-facing docs. Covers context
  vs cognitive load, information hierarchy, completion criteria, leading
  words, the no-op test, and prompt-the-positive. Load by skill name
  (e.g. /skill:writing-for-agents) — no slash-command prompt wrapper.
---

# writing-for-agents: Writing Documents Agents Consume

> Origin: ported from `mattpocock/skills` (`skills/productivity/writing-for-agents/`, MIT). See `THIRD-PARTY-NOTICES.md` at the repo root.
...
Shipped as **`skills/writing-for-agents/SKILL.md`** (+ `SKILL-MECHANICS.md`). There is **no** `prompts/writing-for-agents.md` and **no** `commands/writing-for-agents.md` symlink.

This is a model-invoked reference: the agent loads it when authoring or restructuring agent-facing documents. Do **not** add a thin prompt wrapper "for slash discovery."
