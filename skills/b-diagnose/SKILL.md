...
description: >
  ...
  says "diagnose"/"debug this", or reports something broken, throwing,
  failing, or slow and nobody knows why yet. Owns the gap between "it's
  broken" and b-iterate (defect already written down) / b-build (fix already
  known). Phase 1 (a tight, red-capable feedback loop) is a blocking gate:
  the skill refuses to hypothesize before a named command has gone red on the
  reported bug.
---

# b-diagnose: Diagnosing Bugs

A discipline for hard bugs. Skip phases only when explicitly justified.

> Origin: ported from `mattpocock/skills` (`skills/engineering/diagnosing-bugs/`,
...
touching. Seam vocabulary (**module, interface, seam, adapter, depth,
leverage, locality**) is defined in
[../codebase-design/SKILL.md](../codebase-design/SKILL.md) — use it, link to
...
10. **HITL script.** Last resort. If a human must click, drive _them_ with a
    structured human-in-the-loop script so the loop is still structured and
    captured output feeds back to you. (When `skills/b-wizard/template.sh` is
    present, it is the harness for this — staged progress, confirmation
    gates.)
...
A 50%-flake bug is debuggable; 1% is not, so keep raising the rate until it's
debuggable.
...
codebase architecture is preventing the bug from being locked down. This is
the handoff trigger for `code-smells` (see Exits below) — do not force a fix
through a wrong seam.
...
Where the diagnosis hands off when it terminates:

- **`b-iterate`** — the fix is in place and small; the defect is now written
  down with its regression test. Iterate in place.
- **`b-plan`** — the finding is architectural (the fix reshapes modules or
  crosses seams) and needs its own plan cycle.
- **`code-smells`** — Phase 5's "no correct seam exists" *is itself the
  finding*: the architecture prevents locking this bug down. Hand off to a
  smells/architecture pass rather than forcing a fix through a wrong seam.

Whichever exit fires, the diagnosis record (red command, minimised repro,
ranked hypotheses, winning hypothesis) is the input artifact — write it down
before handing off.
