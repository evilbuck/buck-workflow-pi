...
- To reduce the length of a method body, use [Extract Method](/refactoring/extract-method).
- If local variables and parameters interfere with extracting a method, use [Replace Temp with Query](/refactoring/replace-temp-with-query), [Introduce Parameter Object](/refactoring/introduce-parameter-object) or [Preserve Whole Object](/refactoring/preserve-whole-object).
- If none of the previous recipes help, try moving the entire method to a separate object via [Replace Method with Method Object](/refactoring/replace-method-with-method-object).
- Conditional operators and loops are a good clue that code can be moved to a separate method. For conditionals, use [Decompose Conditional](/refactoring/decompose-conditional). If loops are in the way, try [Extract Method](/refactoring/extract-method).
...
*Source: [Long Method — Refactoring.Guru](https://refactoring.guru/smells/long-method)*
