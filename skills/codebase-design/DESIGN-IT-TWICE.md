...
Spawn 3+ sub-agents in parallel (in OMP, fan out with the `task` tool; in
harnesses without background dispatch, run them as sequential, explicitly
scoped passes). Each must produce a **radically different** interface for the
deepened module.
...
brief so each sub-agent names things consistently with the architecture
