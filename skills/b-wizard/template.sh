...
_existing() {
  [[ -f "$ENV_FILE" ]] || return 1
  local esc_key; esc_key=$(printf '%s' "$1" | sed 's/[][\\.|$(){}?+*^]/\\&/g')
  local line; line=$(grep -E "^${esc_key}=" "$ENV_FILE" | tail -n1) || return 1
  printf '%s' "${line#*=}"
}
...
write_env() {
  local key="$1" value="$2" tmp esc_key
  touch "$ENV_FILE"
  tmp=$(mktemp "${ENV_FILE}.XXXXXX")
  esc_key=$(printf '%s' "$key" | sed 's/[][\\.|$(){}?+*^]/\\&/g')
  grep -vE "^${esc_key}=" "$ENV_FILE" > "$tmp" || true
  printf '%s=%s\n' "$key" "$value" >> "$tmp"
  mv "$tmp" "$ENV_FILE"
  WRITTEN_ENV+=("$key")
...
set_var() {
  local name="$1" value="$2"
  if command -v gh >/dev/null 2>&1 && gh auth status >/dev/null 2>&1; then
    if printf '%s' "$value" | gh variable set "$name" >/dev/null 2>&1; then
