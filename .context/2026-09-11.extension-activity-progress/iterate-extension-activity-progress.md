...
- 2026-09-11 session-end: added 3 `extractErrorMessage` branch tests (error.message object, string error, string result) → `omp-models.ts` patch coverage 88% → 98%. Total patch coverage lifted from 86.5% → 87.8%. Remaining 12.2% gap = 44 instrumented changed lines uncovered, concentrated in `b-commit-improved/index.ts` (29 lines, preflight code 2 path, JSON-parse failure path, `noCache`/`noDraft`/etc. flag-arg paths) and the conflict-resolution loop body in `b-pr-improved/index.ts`. Closing the gap requires constructing preflight-output fixtures per branch — that is a follow-up session, not a one-shot test edit. Decision: leave the tree staged-but-uncommitted; surface to the user for the gate-override decision.
