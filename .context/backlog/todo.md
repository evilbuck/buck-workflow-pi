# Backlog

- [ ] [Test b-grill-auto extension in live Pi session](items/test-b-grill-auto-extension.md)
- [ ] [Redesign b-flow to use Pi SDK for isolated worker contexts](items/b-flow-sdk-redesign.md)

## b-flow SDK Redesign Phases
- [ ] [Phase 3: Test Coverage & Verification](items/phase-3-test-coverage.md)
- [ ] [Add plan-specific implementation ledger for b-review traceability](items/plan-implementation-ledger.md)
- [ ] [Extract b-save prompt into portable skill](items/extract-b-save-skill.md)
